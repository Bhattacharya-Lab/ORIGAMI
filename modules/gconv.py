...
class SVGraphConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # Initialize dummy parameter with size 1 to avoid zero-element tensor warning
    ...
    def forward(self, x: ScalarVector, edge_index, edge_attr: ScalarVector,
        ...
        if edge_index.size(1) == 0:
            return x  # Return input features unchanged for empty graphs
...
class GatConv(MessagePassing):
    ...
    def __init__(self,
        ...
        super().__init__(aggr=aggr, flow='source_to_target')  # (j,i) for updating node i
        self.in_s, self.in_v = in_dims
        self.out_s, self.out_v = out_dims
        self.edge_s, self.edge_v = edge_dims
    ...
    def forward(self, x, edge_index, edge_attr, rot=None):
        ...
        if rot is None:
            rot = torch.eye(3, device=self.dummy.device)
            rot = rot.unsqueeze_(0).repeat(x.v.size(0), 1, 1)  # (N, 3, 3)
...
class GinConv(MessagePassing):
    def __init__(self,
        ...
        self.out_s, self.out_v = out_dims
        self.edge_s, self.edge_v = edge_dims
        self.epsilon = nn.Parameter(torch.zeros(1))  # Initialize with size 1
        ...
        # Initialize dummy parameter with size 1
        self.dummy = nn.Parameter(torch.zeros(1))
