...
class PSRNetwork(nn.Module):
    """Protein Structure Ranking Network with multiple convolution options.
    
    Args:
        node_in_dims (tuple): Input dimensions for node features (scalar_dim, vector_dim).
        node_hid_dims (tuple): Hidden dimensions for node features (scalar_dim, vector_dim).
        edge_hid_dims (tuple): Hidden dimensions for edge features (scalar_dim, vector_dim).
        num_layers (int): Number of graph convolution layers.
        drop_rate (float): Dropout rate.
        top_k (int): Number of nearest neighbors for graph construction.
        num_rbf (int): Number of radial basis functions.
        num_positional_embeddings (int): Number of positional embeddings.
        perceptron_mode (str): Mode for vector perceptron ('svp' or 'gvp').
        conv (str): Type of graph convolution to use:
            - 'gnn': Standard graph neural network
            - 'gat': Graph attention network
            - 'gin': Graph isomorphism network
        num_heads (int): Number of attention heads (only used when conv='gat').
    """
